...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    apt-get update && apt-get install -y libsqlite3-dev sqlite3 libpq-dev firebird-dev default-libmysqlclient-dev freetds-dev
  SHELL
  ...
  #   nb.vm.network "forwarded_port", guest: 4000, host: 4000
  ...
  #   panb.vm.provision "shell", privileged: false, inline: <<-SHELL
  #     # Instalação do Apache 2, Nginx e PHP
  #     sudo apt-get update
